...
void funLang::SemaAnalyzer::actOnFunctionDecl(FunctionDecl *Function, std::unique_ptr<CompoundStmt> CompoundToAttach) {
...
SemaAnalyzer::actOnBooleanLiteral(Token &Literal) {
  assert((Literal.getTag() == Basic::tok::kw_true || Literal.getTag() == Basic::tok::kw_false) && "Token tag is not true or false in actOnBooleanLiteral");
...
Type *SemaAnalyzer::getResultTyForBinOp(Expr *LHS, Expr *RHS) {
  ...
  */
  ...
  if (LHSTy == RHSTy) {// they're both the same, not a problem if they're both literals;
    return LHSTy;
  }
