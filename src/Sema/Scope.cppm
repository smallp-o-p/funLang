...
public:
  ...
  public:
    ...
    [[nodiscard]] Iterator next() const {
      return Iterator(Current->getParent());
    }
  };
...
public:
  Scope(DeclContext *GlobalContext = nullptr)
      : Parent(nullptr), Kind(GlobalScope), ScopeContext(GlobalContext) {}
  Scope(u_ptr<Scope> Parent, const ScopeKind K)
  ...
  [[nodiscard]] Type *getFunctionReturnType() const {
    const auto Res = std::find_if(
        begin(), end(), [&](const Scope &S) { return S.isFunctionScope(); });
    return Res != end()
        ? FunctionDecl::castFromDeclContext(Res->getContext())->getTypePtr()
        : nullptr;
  }
  ...
  Scope *getClosestLoopScope() {
    const auto Res = std::find_if(
        begin(), end(), [&](const Scope &S) { return S.isLoopScope(); });
