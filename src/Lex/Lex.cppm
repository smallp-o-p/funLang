...
#include "Basic/defs/TokenTags.def"
    ...
    constexpr void addKeyword(const std::string &Kw, tok::Tag Tag) {
    ...
    Token lexNum() {
      ...
      while (*End && (isdigit(*End) || *End == '.')) {
        if (SeenDot && *End == '.') {
          Diagnostics.emitDiagMsg(getLocFrom(End + 1),
                                  Diag::err_unexpected_char,
                                  *End);
          return formErr();
        }
        if (*End == '.' && !SeenDot) {
    ...
    Token getNext() {
      ...
      case '=':
        return nextIs('=')
                 ? formToken(BufferPtr + 2, tok::Tag::equalequal)
                 : formToken(BufferPtr + 1, tok::Tag::equal);
      case '.':
        return nextIs('.')
                 ? formToken(BufferPtr + 2, tok::Tag::dotdot)
                 : formToken(BufferPtr + 1, tok::Tag::dot);
      case ':':
        return nextIs(':')
                 ? formToken(BufferPtr + 2, tok::Tag::coloncolon)
      ...
      case '+':
        ...
        return nextIs('+')
                 ? formToken(BufferPtr + 2, tok::Tag::plusplus)
      ...
      case '*': {
        return nextIs('=')
                 ? formToken(BufferPtr + 2, tok::Tag::starequal)
                 : formToken(BufferPtr + 1, tok::Tag::star);
      }
      case '/': {
        return nextIs('=')
                 ? formToken(BufferPtr + 2, tok::Tag::slashequal)
      ...
      default: {
        ...
        Diagnostics.emitDiagMsg(getCurLoc(),
                                Diag::err_unexpected_char,
                                *BufferPtr);
        return formErr();
      }
  ...
  public:
    ...
    /**
      ...
      */
    Token &advance() {
      const Token Tok = UnconsumedTokens.empty() ? getNext() : UnconsumedTokens.front();
