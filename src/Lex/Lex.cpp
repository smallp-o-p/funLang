...
Token Lexer::getNext() {
  ...
  switch (*bufPtr) {
  case ',':
    return formToken(bufPtr + 1, Basic::tok::Tag::comma);
  case '{':
    return formToken(bufPtr + 1, Basic::tok::l_brace);
  case '}':
    return formToken(bufPtr + 1, Basic::tok::r_brace);
  case '(':
    return formToken(bufPtr + 1, Basic::tok::Tag::l_paren);
  case ')':
    return formToken(bufPtr + 1, Basic::tok::Tag::r_paren);
  case '=':
    if (nextIs('=')) {
  ...
  case ';':
    return formToken(bufPtr + 1, Basic::tok::Tag::semi);
  case '!':
    if (nextIs('=')) {
...
bool Lexer::atEnd() { return !*bufPtr; }

Token Lexer::formToken(const char *TokEnd, Basic::tok::Tag Kind) {
  Token Tok = Token{
      llvm::StringRef(bufPtr, static_cast<size_t>(TokEnd - bufPtr)), Kind};
  bufPtr = TokEnd;
  return Tok;
}
...
Token::Token(llvm::StringRef Lexeme, Basic::tok::Tag LexicalTag)
    : Lexeme(Lexeme), LexicalTag(LexicalTag) {}
