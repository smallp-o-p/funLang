...
bool Type::isIntType() const {
  if (const auto *Ty = llvm::dyn_cast<BuiltInType>(OriginalType)) {
    return Ty->getKind() == BuiltInType::i32
        || Ty->getKind() == BuiltInType::i64
        || Ty->getKind() == BuiltInType::int_literal;
  }
  return false;
}

bool Type::isIntButNotLiteral() { return isIntType() and not isIntLiteral(); }
bool Type::isFloatButNotLiteral() {
  return isFloatType() and not isIntLiteral();
}
...
bool Type::isFloatType() const {
  if (const auto *Ty = llvm::dyn_cast<BuiltInType>(OriginalType)) {
    return Ty->getKind() == BuiltInType::f32
        || Ty->getKind() == BuiltInType::f64
        || Ty->getKind() == BuiltInType::float_literal;
  }
  return false;
}
...
bool Type::isPointer() { return llvm::isa<PointerType>(this); }

bool Type::isBuiltIn() { return llvm::isa<BuiltInType>(this); }

Decl *RecordType::lookup(const Symbol *MemberName) const {
  return Record->getFirstDeclName(MemberName);
}
}// namespace funLang
