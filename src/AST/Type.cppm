...
protected:
  ...
  TypeClass Class;
  Type *OriginalType;
  Type(Type *OriginalType, const TypeClass Kind)
      : Class(Kind), OriginalType(OriginalType) {}
...
#include "Basic/defs/BuiltInTypes.def"
  ...
  explicit BuiltInType(const BTKind BuiltIn)
      : Type(this, TK_BUILTIN), BuiltInKind(BuiltIn) {}
  ...
  [[nodiscard]] llvm::StringRef getName() const { return {"type name"}; }
...
public:
  explicit PointerType(Type *PointeeType)
      : Type(this, TK_POINTER), PointeeType(PointeeType) {}
